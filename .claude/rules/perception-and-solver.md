...
**Residual asymmetry (known):** even with full (θ, R) dedup, ~25–30% of the y-flip pixel asymmetry persists in uniform-weight (`angle_weight=None`) cutoff/beta/vonmises sweeps. Traced to `scipy.optimize.root(method='hybr')` itself: `_self_consistent_eq` is y-flip symmetric to 1e-20, but hybr's internal Jacobian estimation uses positive forward-difference steps, so the trajectory from a starting point is not the mirror of its trajectory from the sign-flipped starting point. Two scaffolds for fixing this if it ever matters: symmetrize the multistart by also trying mirrored starts for every candidate, or densify the brentq candidate seeding so both members of a near-SN pair are reachable from independent starts. **It has since mattered:** the deterministic decision-track skeleton (`walker_analysis/decision_skeleton.py`) had its centre route drift off the symmetry axis while riding the SC-*unstable* centre branch (the transverse instability amplifying this asymmetry); patched locally there with a midline pin, but the source fix in `NBM.sc_equilib` (symmetrized multistart) is the real remedy if exact y-symmetry is needed for basins / bifurcation boundaries.
