...
- **Walker blind-spot trap under cutoff weighting — RESOLVED.** The half-angle law removes the behind-walker *dead zone*; the *true blind spot* (zero visible targets) is handled by the `R=0` fast-path, which searches at the independent `walk_std` parameter (default `0.75·π`; see "Euler-Maruyama heading step" / "Blind-spot search"). Background in [weighting_analysis/README.md](weighting_analysis/README.md).
