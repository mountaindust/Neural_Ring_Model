...
**Blind-spot search (`blind_search_std`, default 0.75·π).** On any step where *no* target is visible — `percep_model.get_neural_signals()` returns empty arrays (e.g. under a cutoff weight with `b_weight < π`, all targets fall outside the visible cone) — γ collapses to 0 and the deterministic torque vanishes, so the small `std` would leave the heading frozen and the walker would drift straight off and never re-acquire. Instead, on a blind step the walker takes a heading kick `convert_angles(focal_angle + rng.normal(blind_search_std)·√dt)` and resets `self.gamma = 0`, turning into a pure rotational-diffusion search walk until a target re-enters view (the proximity/trajectory checks still fire normally). The `√dt` scaling matches the Euler-Maruyama noise term in the deterministic branch (see "Euler-Maruyama heading step" below), so the search is dt-invariant. Set `blind_search_std=0` to recover the frozen-drift behavior. Validated in [tests/test_half_angle_torque.py](tests/test_half_angle_torque.py): a lone-target blind start is re-acquired across seeds; frozen drift marches straight off.
...
**Euler-Maruyama heading step.** The heading update is `θ ← θ + dθ/dt·dt + std·√dt·Z` with `Z ~ N(0,1)` (drawn via `rng.normal(scale=std)`, where `scale` is the standard deviation — `noise·√dt` in code). The diffusion term scales as **`√dt`, not `dt`**: a Wiener increment has variance `dt`, so `std·√dt·Z` gives a per-unit-time angular variance of `std²` that is **independent of the step size**. An earlier version multiplied the noise by `dt` (variance `∝ dt²`), which made the effective noise temperature silently shrink as `dt` was refined; the `√dt` form is dt-invariant. `std` is therefore a rotational-diffusion intensity, not a per-step angular standard deviation. The deterministic stability machinery (SC equilibria, Jacobians, bifurcation rasters) is noise-free and untouched by this.
