...
- [VM_bifurcation_old_dtheta/](VM_bifurcation_old_dtheta/) — bifurcation/stability exploration scripts + write-up. Script roles + the physical-phenomena findings are in the auto-loading `.claude/rules/bifurcation-explorations.md`. Not publication-quality — sized for fast iteration on a many-core machine. (`bifurc_plots/` was removed 2026-08-19: two of its three scripts called the deleted `'coupled'` criterion, and the third was a low-resolution duplicate of [plots/neural_weight_sweep.py](plots/neural_weight_sweep.py).)
...
| Bifurcation-script roles + physical phenomena (decision paralysis) | `VM_bifurcation_old_dtheta/` | `.claude/rules/bifurcation-explorations.md`; [VERDICT.md](VM_bifurcation_old_dtheta/VERDICT.md) |
