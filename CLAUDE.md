...
- [VM_bifurcation_old_dtheta/](VM_bifurcation_old_dtheta/) — diagnostic scripts and the [VERDICT.md](VM_bifurcation_old_dtheta/VERDICT.md) write-up of the Hopf-island / saddle-node bifurcation skeleton near (2.1, ±2.45) in the vonmises-k0.55 / two-target setup. **Old-law (`sin(ego)`) analysis** — see [reduced_criterion/](reduced_criterion/) for the current half-angle-law picture.
- [reduced_criterion/](reduced_criterion/) — analysis of the `'reduced'` (timescale-separated) stability criterion vs `'coupled'`, and the dynamics where they differ. Canonical scripts + figures + [README.md](reduced_criterion/README.md); the block-determinant-identity derivation is in [block_determinant_identity.md](reduced_criterion/block_determinant_identity.md).
...
The legacy 2D criterion (`_discrim_A` on NBM, `_discrim_A_nu` on IEM) *over-counts* stable equilibria — at (1.5, 0) in the vonmises k=0.55 setup, both `reduced` and `coupled` report 3 stable while `discrim_a` reports 5. The 2 extra `discrim_a`-"stable" equilibria at `θ ≈ ±0.16` are **fast-stable but slow-unstable**: the γ block `A` is Hurwitz, yet the slow Schur complement `λ_slow > 0` (≈ +0.26 under the current half-angle law; the matching positive eigenvalue in the full 3×3 is ≈ +0.18 — the finite-ε rescaling of the same root; was ≈ +0.31 under the old `sin(ego)` law). This is exactly the slow heading-tracking instability `discrim_a` cannot see. At (1.5, 0) `reduced` and `coupled` agree (no Hopf there); they diverge only inside Hopf regions, where `coupled` flags an oscillatory instability and `reduced` does not.
...
- **Walker blind-spot trap under cutoff weighting — RESOLVED.** The half-angle law removes the behind-walker *dead zone*; the *true blind spot* (zero visible targets) is handled by the `R=0` fast-path, which searches at the independent `walk_std` parameter (default `0.5·π`; see "Euler-Maruyama heading step" / "Blind-spot search"). Background in [weighting_analysis/README.md](weighting_analysis/README.md).
