...
**Blind-spot search (`blind_search_std`, default π/2).** On any step where *no* target is visible — `percep_model.get_neural_signals()` returns empty arrays (e.g. under a cutoff weight with `b_weight < π`, all targets fall outside the visible cone) — γ collapses to 0 and the deterministic torque vanishes, so the small `std` would leave the heading frozen and the walker would drift straight off and never re-acquire. Instead, on a blind step the walker takes a large heading kick `convert_angles(focal_angle + rng.normal(blind_search_std))` (NOT scaled by `dt`) and resets `self.gamma = 0`, turning into a diffusive search walk until a target re-enters view (the proximity/trajectory checks still fire normally). Set `blind_search_std=0` to recover the old frozen-drift behavior. Validated in [tests/test_half_angle_torque.py](tests/test_half_angle_torque.py): a lone-target blind start is re-acquired in ~20–35 steps across seeds; frozen drift marches straight off.
