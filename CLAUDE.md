...
- [stale_coupled_model_starting_code/](stale_coupled_model_starting_code/) — **stale.** Scripts written against the removed `'coupled'` criterion; kept only as starting code for an eventual `(n⃗, θ)` population-level comparison. See its README. (The other two exploration directories, `bifurc_plots/` and `VM_bifurcation_old_dtheta/`, were **deleted** 2026-08-19 — coupled-criterion scripts, a duplicate of [plots/neural_weight_sweep.py](plots/neural_weight_sweep.py), and a write-up whose surviving finding is now carried by `.claude/rules/torque-and-stability.md` and [tests/test_reduced_criterion.py](tests/test_reduced_criterion.py).)
