...
**Blind-spot search (via the `R=0` fast-path, at a fixed floor).** On any step where *no* target is visible — `percep_model.get_neural_signals()` returns empty arrays (e.g. under a cutoff weight with `b_weight < π`, all targets fall outside the visible cone) — a cheap fast-path sets `self.gamma = 0`, deterministic torque `= 0`, `R = 0` (no wasted `dγ/dt` solve), and the heading noise to **`max(std, 0.75·π)`** — a pure-rotational-diffusion search walk until a target re-enters view (proximity/trajectory checks still fire normally). The `0.75·π` **floor is decoupled from the committed `std`** on purpose: the gated formula `σ·(1−R)^p` gives only `σ` at `R=0`, which in gentle constant mode (`std=0.1`) is far too weak to re-acquire — so the floor guarantees a vigorous search regardless of mode. The dedicated `blind_search_std` knob was **removed** (the floor is the module constant `_BLIND_SEARCH_STD`, not a per-call knob). `std=0` keeps the frozen-drift (deterministic) behavior. Validated in [tests/test_half_angle_torque.py](tests/test_half_angle_torque.py): a lone-target blind start re-acquires under the *default* constant mode (`std=None→0.1`); `std=0` marches straight off.
...
- **Regime-aware default for `std=None`:** `0.1` when `noise_exp==0` (a gentle constant noise) and `0.75·π` when `noise_exp>0` (the random-walk intensity the gate tames once committed). `σ` is the *visible*-step scale; the default picks the right one. `std=0` ⇒ fully deterministic. **Blind steps ignore this and search at `max(std, 0.75·π)`** (see "Blind-spot search") — the `R=0` value of `σ·(1−R)^p` would otherwise be only `σ`, too weak in constant mode.
...
- **Walker blind-spot trap under cutoff weighting — RESOLVED.** The half-angle law removes the behind-walker *dead zone*; the *true blind spot* (zero visible targets) is handled by the `R=0` fast-path, which searches at the fixed floor `max(std, 0.75·π)` (see "Euler-Maruyama heading step" / "Blind-spot search"). The old dedicated `blind_search_std` knob was removed (replaced by the module constant `_BLIND_SEARCH_STD`). Background in [weighting_analysis/README.md](weighting_analysis/README.md).
