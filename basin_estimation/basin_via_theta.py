...
def _eval_f(theta, gamma_eq):
    ...
    # Half-angle torque law (matches NeuralBandModel.dtheta_dt). ego is in
    # [-pi, pi] from get_neural_angle_inverse(np.angle(.)); the +-pi branch cut
    # is the intentional jump discontinuity (Step-7 detector catches it).
    return nbm.K * R * np.sin(ego/2)
