...
- **Outer born = a saddle-node.** A genuinely *new* off-axis equilibrium, locked onto
  ...
  transition** that requires strong commitment (low temperature T, large outer-target
  weight ρ, well-resolved separation). It is **marginal at the default** (it flickers
  in/out at the solver's resolution) and **vanishes entirely** for T ≳ 0.22, narrow
...
γ = Σ ρ_k e^{iν_k} σ_k with an **Ising magnetization** σ_k = σ(2NR cos ν_k / T)
...
```
destabilizing (magnetization feedback):  + (2NR/T) · sigma'(z0) · sin^2(nu0)
...
large neural angle (≈ −110°, cos < 0) where the Ising logistic σ(2NR cos ν/T) ≈ 0
...
mean-field magnetization: it exists only once ρ_outer · (N/T) · R clears the ordering
...
|---|---|---|---|
| **T** 0.10→0.40 | 2.55 → 2.85 (robust) | 2.67 → **vanishes for T≳0.22** | the outer is an Ising transition: T-fragile; center is not |
...
it only weakly. (In the instability condition `(2NR/T)σ′sin²ν vs σcos ν` the explicit ρ
...
heading (N=2), which is exactly what lets the outer lock; widening or narrowing it
...
Same center pitchfork embedded in a dense multi-target cascade (see
[branch_diagram_ninegon.png](branch_diagram_ninegon.png)): the 0° direction is robust
except for an unstable interlude (x ≈ 2.0–2.5) and is reborn by x ≈ 3.0 — exactly the
forward-three-target pitchfork — while the many side/rear targets each contribute their
own branches that are born and die as the observer moves.
...
(T=0.2, 40°, r=5, D=5) the outer saddle-node sits right at threshold — born late
(x≈3.05) and only briefly (it flickers). To make the outer tracks **robust and
early**: lower T, widen the separation, raise ρ_outer (larger targets / wider weight
window), or shrink D. To **suppress** them: raise T, narrow the separation, or widen
...
**T, the warp, and the weight window**. Figure
[birth_mechanism_knobs.png](birth_mechanism_knobs.png) (extended-T + gap + a_weight).

- **Temperature T is the clean, dominant knob.** The gap (outer − reborn) shrinks
  **roughly linearly with T**, and the outer **backs right up against the pitchfork**:

  | T | center reborn | outer born | gap |
  |---|---|---|---|
  | 0.025 | 2.35 | 2.39 | **+0.04** |
  | 0.05  | 2.43 | 2.49 | +0.06 |
  | 0.10  | 2.55 | 2.67 | +0.12 |
  | 0.20 (default) | 2.73 | 3.05 | +0.32 |

  Lowering T brings **both** births in *and together*, so all three stable directions
  become available at nearly the same observer x. Mechanism: the outer is an Ising
  ordering transition with threshold ∝ 1/T, so colder → commitment is cheaper → the
  outer lock appears at lower R (smaller x), sliding toward the nearly-T-independent
  pitchfork.

- **The gap does not go negative.** Across T and combined low-T + warp/weight sweeps
  the smallest gap reached was ≈ +0.02 (e.g. T=0.025, a_weight=0.2π → reborn 2.33,
...
- **Lowering `a_warp` shifts the whole cascade earlier in x** (both births) while
  keeping the gap small — e.g. T=0.05, a_warp=0.40π → reborn 2.21, outer 2.25
...
- **`a_weight` and `b_weight` are weak / already-optimal.** Lowering `a_weight` pulls
  the outer in only modestly and at low T mostly moves the *center* (counterproductive
